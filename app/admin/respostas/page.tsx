...
interface Props { searchParams: { vaga?: string } }
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <Link
          ...
          className={`tag text-xs py-1 px-3 transition-colors ${
            !vagaFiltro ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
          }`}
        >
          Todas
        </Link>
        {todasVagas.map(v => (
          <Link
            ...
            className={`tag text-xs py-1 px-3 transition-colors ${
              vagaFiltro === v.id ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}
          >
            {v.titulo}
          </Link>
      ...
      {sessoes.length === 0 ? (
        ...
        <div className="space-y-4">
          {sessoes.map((sessao, i) => {
            ...
            return (
              <details key={sessao.sessao_id} className="card overflow-hidden group">
                <summary className="px-5 py-4 flex items-center gap-4 cursor-pointer hover:bg-gray-50/60 transition-colors list-none">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 font-mono truncate">{sessao.sessao_id}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="text-xs text-gray-400">{formatDate(sessao.criada_em)}</p>
                    <p className="text-xs text-gray-300 mt-0.5">
                      {sessao.respostas.length} resposta{sessao.respostas.length !== 1 ? 's' : ''}
                    </p>
                  </div>
                  <span className="text-gray-300 group-open:rotate-180 transition-transform ml-1 text-xs">▼</span>
                </summary>
                ...
                <div className="border-t border-gray-50 divide-y divide-gray-50">
                  {sessao.respostas
                    ...
                    .map(resp => {
                      ...
                      const pergunta = (vaga?.perguntas ?? []).find(p => p.id === resp.pergunta_id)
