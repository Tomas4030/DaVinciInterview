...
export default async function AdminCompanyDashboardPage({ params }: Props) {
  ...
  const [createdWindowRows] = await query<{ week_count: number; month_count: number }>(
    `
    SELECT
  ...
  const completionRate = totalResponses > 0 ? Math.round((totalCompleted / totalResponses) * 100) : 0;
    ...
    <section className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <article className="rounded-xl border border-[var(--c-border)]/70 bg-[var(--c-surface)] p-4">
          ...
          <p className="mt-1 text-xs text-[var(--c-muted)]">{interviewsWeek} na semana, {interviewsMonth} no mês</p>
        </article>
        <article className="rounded-xl border border-[var(--c-border)]/70 bg-[var(--c-surface)] p-4">
        ...
        <article className="rounded-xl border border-[var(--c-border)]/70 bg-[var(--c-surface)] p-4">
          ...
          <p className="mt-1 text-xs text-[var(--c-muted)]">Taxa de conclusão: {completionRate}%</p>
        </article>
      </div>
      ...
      <div className="rounded-xl border border-[var(--c-border)]/70 bg-[var(--c-surface)] p-5">
        <div className="mb-3 flex items-center justify-between gap-2">
          ...
          <Link href={`/admin/${params.slug}/interviews`} className="text-xs text-[var(--c-muted)] hover:text-[var(--c-text)]">
            Ver todas
          </Link>
        </div>
        ...
        {recentInterviews.length === 0 ? (
          ...
          <div className="divide-y divide-[var(--c-border)]/50">
            {recentInterviews.map((item) => (
              <article key={item.id} className="flex flex-wrap items-center justify-between gap-2 py-3">
                <div>
      ...
      <div className="rounded-xl border border-[var(--c-border)]/70 bg-[var(--c-surface)] p-5">
        <h2 className="text-base font-semibold text-[var(--c-text)]">Plano atual</h2>
        <p className="mt-2 text-sm text-[var(--c-muted)]">
          Plano: <span className="font-medium text-[var(--c-text)]">{membership.company.plan}</span>
        </p>
        <p className="text-sm text-[var(--c-muted)]">
          Subscrição: <span className="font-medium text-[var(--c-text)]">{membership.company.subscription_status}</span>
        </p>
      </div>
