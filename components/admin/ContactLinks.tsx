"use client";

function MailIcon() {
  return (
    <svg
      width="14"
      height="14"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      aria-hidden="true"
    >
      <path d="M4 4h16v16H4z" opacity="0" />
      <path d="M4 6h16v12H4z" />
      <path d="m22 7-10 7L2 7" />
    </svg>
  );
}

function PhoneIcon() {
  return (
    <svg
      width="14"
      height="14"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      aria-hidden="true"
    >
      <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.86 19.86 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6A19.86 19.86 0 0 1 2.08 4.18 2 2 0 0 1 4.06 2h3a2 2 0 0 1 2 1.72c.12.9.33 1.78.62 2.62a2 2 0 0 1-.45 2.11L8 9.91a16 16 0 0 0 6.09 6.09l1.46-1.23a2 2 0 0 1 2.11-.45c.84.29 1.72.5 2.62.62A2 2 0 0 1 22 16.92z" />
    </svg>
  );
}

export function ContactLinks({
  email,
  telefone,
}: {
  email?: string | null;
  telefone?: string | null;
}) {
  if (!email && !telefone) {
    return (
      <span className="text-[11px] text-[var(--c-muted)]/60 italic">
        sem contacto
      </span>
    );
  }

  return (
    <>
      {email && (
        <a
          href={`mailto:${email}`}
          className="inline-flex items-center gap-1.5 rounded-full bg-[var(--c-bg)] px-2.5 py-1 text-[11px] font-medium text-[var(--c-muted)] ring-1 ring-[var(--c-border)]/60 transition hover:text-[var(--c-text)] hover:ring-[var(--c-border)]"
          onClick={(e) => e.stopPropagation()}
        >
          <MailIcon />
          {email}
        </a>
      )}

      {telefone && (
        <a
          href={`tel:${telefone}`}
          className="inline-flex items-center gap-1.5 rounded-full bg-[var(--c-bg)] px-2.5 py-1 text-[11px] font-medium text-[var(--c-muted)] ring-1 ring-[var(--c-border)]/60 transition hover:text-[var(--c-text)] hover:ring-[var(--c-border)]"
          onClick={(e) => e.stopPropagation()}
        >
          <PhoneIcon />
          {telefone}
        </a>
      )}
    </>
  );
}
