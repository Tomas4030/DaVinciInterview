...
function MensagemItem({ msg }: { msg: Mensagem }) {
  ...
  return (
    ...
    >
      ...
      <div className={`max-w-[78%] ${isBot ? 'bubble-bot' : 'bubble-user'} px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap`}>
        {msg.texto}
      </div>
...
export default function ChatEntrevista({ vaga }: { vaga: Vaga }) {
  ...
  const botResponde = useCallback((texto: string, delay = 700) => {
  ...
  async function enviarResposta() {
    ...
    if (proximoIndice < vaga.perguntas.length) {
      ...
      await botResponde(
        `Entrevista concluída! 🎉\n\nObrigado pela tua participação, ${vaga.titulo}. As tuas respostas foram guardadas e serão analisadas pela nossa equipa em breve.\n\nBoa sorte! 🍀`
      )
      setConcluida(true)
    }
  ...
  const statusLabel = indiceAtual < 0
    ? 'Pronto para começar'
    : concluida
    ? 'Concluída ✓'
          ...
          <div className="flex items-center gap-3 mb-2.5">
            ...
            <div className="flex items-center gap-2 min-w-0">
              <div className="w-6 h-6 rounded-lg bg-brand-600 flex items-center justify-center flex-shrink-0">
                <span className="text-white text-[10px] font-bold font-display">D</span>
              </div>
              <p className="text-sm font-semibold text-gray-900 truncate">{vaga.titulo}</p>
            </div>
            <span className="ml-auto text-xs text-gray-400 flex-shrink-0">{statusLabel}</span>
          </div>
          <ProgressBar current={perguntasRespondidas} total={vaga.perguntas.length} />
        </div>
      </header>
        ...
        <div className="max-w-2xl mx-auto px-4 py-3">
          ...
          {indiceAtual === -1 && !concluida ? (
            ...
            <div className="flex gap-3">
              <Link href="/" className="btn-ghost flex-1 py-3 text-sm text-center">
                ← Ver outras vagas
              </Link>
            </div>
